...
class homepage_session_controller(request_handler):
    ...
    def __init_parameters(p_data):
        results_dict_day = ast.literal_eval(
            redis_controller().invoke_trigger(
                REDIS_COMMANDS.S_GET_STRING,
                [REDIS_KEYS.INSIGHT_STAT_DAY, REDIS_DEFAULT.INSIGHT_STAT_DEFAULT, None]
            )
        )
        results_dict_week = ast.literal_eval(
            redis_controller().invoke_trigger(
                REDIS_COMMANDS.S_GET_STRING,
                [REDIS_KEYS.INSIGHT_STAT_WEEK, REDIS_DEFAULT.INSIGHT_STAT_DEFAULT, None]
            )
        )
        ...
        m_context = {
            ...
            HOMEPAGE_CALLBACK.M_STATISTICS: combined_statistics,
        }
